...
absl.flags.DEFINE_string("filter_prefix", '', "optional prefix to filter models by start of filenames") # same_class, different_class, baseline
...
def run_experiment(path:str,dataset_dir:str):
    ...
    if os.path.isdir(path):
        list_models = [name_file for name_file in os.listdir(path) if (name_file.endswith('.pt') and name_file.startswith(FLAGS.filter_prefix))]
    else:
    ...
    for _, name_model in enumerate(sorted(list_models)):
        # load model
        run_str = os.path.splitext(name_model)[0]
        run = int(run_str.split('_')[-1]) - 1
        utils.set_seed(run)
