...
class MemoryStrategy:
    ...
    def build(self, sample: SelectedSample, baseline_memory: torch.Tensor, class_to_dataset_indices: Dict[int, List[int]], memory_dataset: torch.utils.data.Dataset, memory_size: int, num_classes: int) -> Tuple[torch.Tensor, List[int]]:
...
class DifferentClassStrategy(MemoryStrategy):
    ...
    def build(self, sample: SelectedSample, baseline_memory: torch.Tensor, class_to_dataset_indices: Dict[int, List[int]], memory_dataset: torch.utils.data.Dataset, memory_size: int, num_classes: int) -> Tuple[torch.Tensor, List[int]]:
        ...
        chosen_label = self.rng.choice(candidates)
...
def save_image(image_tensor: torch.Tensor, path: str, undo_normalize_fn) -> None:
    """Convert tensor image to uint8 image and save it to the disk."""
    # Undo normalization and clamp to [0,1]
    img = undo_normalize_fn(image_tensor.detach().cpu())
    img = img.clamp(0, 1).permute(1, 2, 0).numpy()
    # Scale pixels and save
...
def collect_selected_samples(model: torch.nn.Module, test_loader: torch.utils.data.DataLoader, baseline_memory: torch.Tensor, device: torch.device) -> List[SelectedSample]:
...
def run_analysis(args: argparse.Namespace) -> None:
    ...
    if dataset_name != checkpoint["dataset_name"]:
        raise ValueError(f"Dataset mismatch. checkpoint dataset={checkpoint['dataset_name']}, --dataset={dataset_name}")
    ...
    if len(selected_samples) < 4:
        raise RuntimeError(f"Could not find 4 selected samples (2 correct + 2 wrong). Found {len(selected_samples)}.")
            ...
            for sample in selected_samples:
                ...
                for strategy in strategies:
                    ...
                    for rank, mem_pos in enumerate(top3_pos, start=1):
                        ...
                        out_path = os.path.join(images_dir, f"sample_{sample.sample_index}_{strategy.name}_top{rank}_mempos_{mem_pos}.png")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--small_memory_size", type=int, default=2, choices=[1, 2], help="Small memory size for degenerate strategy")
